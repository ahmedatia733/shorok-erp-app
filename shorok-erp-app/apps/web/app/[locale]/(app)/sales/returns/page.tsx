...
import { HistoricalReturnsSection } from "../../../../../components/returns/historical-returns-section";
...
// The archive lives behind its own tab so an operational return and a historical
// one can never be mistaken for each other: they never share a table, and the
// create/confirm/cancel controls belong to the operational tab alone.
type Section = "operational" | "historical";
...
export default function SalesReturnsPage() {
  ...
  const [section, setSection] = useState<Section>("operational");
  ...
  const tabClass = (active: boolean) =>
    `px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors ${
      active ? "border-primary text-primary" : "border-transparent text-textSecondary hover:text-textPrimary"
    }`;
  ...
  return (
    <div className="space-y-4" dir="rtl">
      <div className="flex items-center justify-between">
        ...
        {/* The archive can never be created, so the action lives on the operational tab only. */}
        {section === "operational" && canCreate && <a href={`/${locale}/sales/returns/new`}><Button>مردود جديد</Button></a>}
      </div>

      <div role="tablist" aria-label={t("sectionsLabel")} className="flex gap-2 border-b border-border">
        <button
          type="button"
          role="tab"
          aria-selected={section === "operational"}
          onClick={() => setSection("operational")}
          className={tabClass(section === "operational")}
          data-testid="tab-operational-returns"
        >
          {t("tabOperational")}
        </button>
        <button
          ...
          className={tabClass(section === "historical")}
          data-testid="tab-historical-returns"
        >
          {t("tabHistorical")}
        </button>
      </div>
